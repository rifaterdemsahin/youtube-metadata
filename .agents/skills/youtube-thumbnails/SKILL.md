...
2. **Commit it to this repo.** `git add thumbnails/thumbnail_<VIDEO_ID>.jpg` and include it in the metadata-package commit. Custom thumbs must not stay untracked. Do not commit `frame_*` stills or live CDN copies (`live_*.jpg`).
...
5. **Verify on YouTube.** Download the live `maxresdefault` (cache-bust with `?t=`) and `read_file` it. Compare to the local jpg. API `thumbnails.set` returning success is not enough — Shorts often keep an auto frame.
